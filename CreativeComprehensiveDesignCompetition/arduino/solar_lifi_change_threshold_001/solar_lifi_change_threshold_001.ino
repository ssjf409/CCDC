...
int thresholds[20];
long thresholds_sum = 0;

int recVal1 = 0;
int recVal2 = 0;
 

unsigned long INTERVAL  = 15000; //30000
...
IPAddress server(106,10,50,11);  // 네이버 사전 IP 주소 (DNS 사용 않함) 
...
void loop() {
  ...
  if(mode == 1) {
    ...
    if (client.connect(server, 3000)) { 
      ...
      mode = 2;
      // HTTP request를 만듭니다: 
      client.println("GET /model/modelKey/2"); //2는 회사입구
...
//        thresholds_sum = thresholds_sum - thresholds[i] + sensorValue;
//        thresholds[i] = sensorValue;
//        threshold = thresholds_sum / 20  + 10;
        sensorValue = analogRead(sensorPin);
        threshold = sensorValue + 20;
        Serial.println(threshold);
      ...
      for(unsigned long i = 0; i < dsize; i++) // dsize - > dsize + 2
      {
        c_timer = millis();       //== test
        while(!moveToNextBit)
        {
      ...
      if(recVal1 == 255 && recVal2 == 255) {
        threshold = (threshold + sensorValue) / 2;
      }
      Serial.print("recVal1 : ");
      Serial.println(recVal1);
      Serial.print("recVal2 : ");
      Serial.println(recVal2);
